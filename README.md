...
Claude Code 기반 AI 개발 워크플로우를 표준화한 하네스. 서브에이전트·슬래시 커맨드·훅을 한 저장소로 묶어 여러 머신 간 동기화하고, 기능 개발부터 코드 리뷰·검증까지 자동화합니다.

## 구성
- **전문 에이전트 4종**: analyzer / implementer / code-reviewer / verifier
- **슬래시 커맨드 8종**: 작업 오케스트레이션, 분석, 리뷰, 커밋, todo, 리포트, 위키 초기화 등
- **백그라운드 위임**: 에이전트가 비동기로 돌며 사용자에게 제어권 반환
- **리포트 관리**: 에이전트 산출물을 채팅이 아닌 위키에 저장
